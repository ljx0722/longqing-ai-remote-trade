...
var cityPrice = (city) => (city.coastal ? 1800 : 1250) + city.era * 1400 + hash2(city.id) % 650;
var cityDefense = (city) => 14 + city.era * 3 + hash2(city.id) % 14;
...
var Dominion = class {
  ...
  attack(id) {
    ...
    this.state.campaign = {
      ...
      roll: hash2(`${id}:${this.host.day}:${serial}:${this.state.troops}`) / 4294967296,
      serial
    };
...
function readDominionState(raw, day) {
  ...
  ) || !integer(state.troops, 0, 100) || !integer(state.campaignSerial, 0, 1e7) || !integer(state.caravanSerial, 0, 1e7) || !integer(state.lastProcessedDay, -1, day))
    return null;
  for (const [id, value] of Object.entries(state.owned)) {
    if (!cityById.has(id) || !record(value) || !integer(value.commerce, 0, 3) || !integer(value.food, 0, 3) || !integer(value.defense, 0, 3) || !integer(value.loyalty, 0, 100) || !integer(value.acquiredDay, 0, day) || !["charter", "conquest"].includes(value.method))
      return null;
  ...
  for (const caravan of state.activeCaravans) {
    ...
    if (!road || !state.owned[caravan.fromId] || !(road.a === caravan.fromId && road.b === caravan.toId || road.b === caravan.fromId && road.a === caravan.toId) || !integer(caravan.sentDay, 0, day) || !integer(
      caravan.returnsDay,
      caravan.sentDay + 1,
      caravan.sentDay + 100
    ) || !integer(caravan.reward, 1, 2e4))
      return null;
  ...
  if (state.campaign !== null) {
    ...
    if (!record(campaign) || typeof campaign.targetId !== "string" || !cityById.has(campaign.targetId) || state.owned[campaign.targetId] || typeof campaign.fromId !== "string" || !state.owned[campaign.fromId] || !portById.has(campaign.fromId) || !integer(campaign.sentDay, 0, day) || !integer(
      campaign.arrivesDay,
      campaign.sentDay + 1,
      campaign.sentDay + 100
    ) || !integer(campaign.force, 1, 100) || !integer(campaign.serial, 1, state.campaignSerial) || typeof campaign.winChance !== "number" || !Number.isFinite(campaign.winChance) || campaign.winChance < 0.025 || campaign.winChance > 0.95 || typeof campaign.roll !== "number" || !Number.isFinite(campaign.roll) || campaign.roll < 0 || campaign.roll >= 1)
      return null;
