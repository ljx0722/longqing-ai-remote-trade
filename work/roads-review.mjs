...
test("32 distinct inland cities use real coordinates and never replace ports", () => {
  assert.ok(inlandCities.length >= 24);
  assert.equal(
...
test("Explicit caravan roads stay on continental land, without ocean jumps", () => {
  ...
  for (const road of inlandRoads) {
    ...
    if(samples.filter(Boolean).length / samples.length < 0.7) console.log("WATER",road.id,JSON.stringify(road.waypoints));
  }
});
